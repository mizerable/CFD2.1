...
instance Control.DeepSeq.NFData (Equation (Term Double)) 
instance Control.DeepSeq.NFData Property
instance Control.DeepSeq.NFData Direction
instance Control.DeepSeq.NFData Side

data Sign = Positive | Zero | Negative deriving (Enum,Ord,Eq,Show)

data Side =  Now | Prev |East | West | North | South | Top | Bottom | Center 
    deriving (Show,Eq,Ord, Enum, Generic)
  
data SchemeType = Directional | Nondirectional       
  
data Direction = Time | X | Y | Z deriving (Enum,Ord,Eq,Show, Generic)

data DimensionType = Temporal | Spatial deriving ( Eq)
data Equation  a= Equation{
    lhs:: ![a]
    ,rhs:: ![a]
    ,unknownProperty:: !Property} deriving (Generic)    
    
data IntegralType = Body | Surface deriving (Eq)
data Property = Speed | Vorticity | Pressure| U | V | W | Density | Temperature | Mew | Dye 
    deriving (Ord,Eq,Enum,Show, Generic)
...
data Expression a = Expression{getTerms:: ![Term a]} deriving (Generic) 
...
instance Control.DeepSeq.NFData (Term Double)
instance Control.DeepSeq.NFData (Expression Double)
...
boundaryPair d = case d of 
     ...
     Y -> (North,South)
     Z -> (Top,Bottom)
     Time ->(Now,Prev) -- error "tryng to detect where this is used"
...
offsetPosition_adj (idx,t) side env = case side of
    Center -> (idx,t) 
    Now -> (idx,t) 
    Prev -> (idx, pushBackTime t) -- error "can't push back to another time level"    
    _ -> 
